...
function startRound(){
	...
	function renderHands(hideDealerHole=true){
	  ...
	  playerHand.forEach(c=> playerCardsEl.appendChild(renderCard(c)));
	  dealerScoreEl.textContent = inRound ? (hideDealerHole? '?' : String(scoreHand(dealerHand))) : '--';
	  playerScoreEl.textContent = playerHand.length? String(scoreHand(playerHand)) : '--';
	}
	...
	function startRound(){
	  deck = createDeck(); shuffle(deck);
	  playerHand = []; dealerHand = [];
	  inRound = true; if(bjLog) bjLog.innerHTML='';
	  // deal sequence with small delays
	  dealCard(playerHand); renderHands(true);
	  setTimeout(()=>{ dealCard(dealerHand); renderHands(true); }, 200);
	  setTimeout(()=>{ dealCard(playerHand); renderHands(true); }, 400);
	  setTimeout(()=>{ dealCard(dealerHand); renderHands(true); log('ゲーム開始 — ヒットまたはスタンドを選択してください'); }, 600);
	}

	function playerHit(){
	  if(!inRound) return; dealCard(playerHand); renderHands(true); playSound('click');
	  const s = scoreHand(playerHand);
	  if(s>21){ playSound('lose'); endRound('バースト — あなたの負け'); }
	}
	...
	if(bjNew) bjNew.addEventListener('click', ()=> startRound());
	if(bjHit) bjHit.addEventListener('click', ()=> playerHit());
	if(bjStand) bjStand.addEventListener('click', ()=>{ if(!inRound) return; endRound(); });
